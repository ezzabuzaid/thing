...
export function getCurrentCountry(): Country | undefined {
  if (typeof navigator === 'undefined') return undefined; // -- SSR guard
  const locale = navigator.language ?? 'en-US'; // e.g. "fr-FR"
  const region = new Intl.Locale(locale).region; // "FR"
...
>(({ className, ...props }, ref) => (
  <Input
    className={cn(
      'rounded-s-none rounded-e-lg focus-visible:ring-[0px]',
      className,
    )}
    {...props}
    ref={ref}
